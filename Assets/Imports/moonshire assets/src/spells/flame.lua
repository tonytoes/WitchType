...
function spawnFlame(x, y)
    ...
    function flame:singleEmber(newVec, offVec, offMag)
        for i=1,2 do
            ...
            if i == 1 then effects:spawn("flameSmoke", self.x + newVec.x + offX + offVec.x, self.y + newVec.y + offY + offVec.y, {scale = 0.9}) end
            local offX = math.random()*rad
            local offY = math.random()*rad
    ...
    function flame:hit()
        ...
        if polygon then
            local hitEnemies = world:queryPolygonArea(polygon, {'Enemy', 'Enemy2'})
            for _,e in ipairs(hitEnemies) do
                local knockbackDir = self.dir
                e.parent:burn()
                local xtra = 0
                if math.random() > 0.5 then xtra = 1 end
                e.parent:hit(1, nil, 0.06, nil, true)
            end

            local walls = world:queryPolygonArea(polygon, {'Wall', 'Ground'})
