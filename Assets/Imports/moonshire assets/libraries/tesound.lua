...
dj.tweens = {} -- Keeps track of tweens for all channels
...
function dj.play(sound, sourceType, tags, volume, pitch, func, loop)
  if not sound then print("Tried to play a sound, but it was nil") return end
  ...
  local volMult = 1
  if tags == "effect" then
    volMult = settings.soundVolume * settings.soundMultiplier
  end
  ...
  if loop then s[1]:setLooping(true) end
  s[1]:play()
  s[1]:setVolume( (volume or 1) * volMult * dj.findVolume(tags) * (dj.volumeLevels.all or 1) )
...
function dj.playLoopingSeamless(sound, sourceType, tags, n, volume, pitch)
  return dj.play( sound, sourceType, tags, volume, pitch,
    nil, true)
end
...
function dj.stop(channel, finish)
  ...
  elseif type(channel) == "string" then
    if channel == "music" then music.currentSong = "" end
    for k,v in pairs(dj.findTag(channel)) do dj.stop(v, finish) end
  end
end
...
function dj.fade(time)
  if not time then time = 1 end
  if settings.musicMaxVolume <= 0 then return end
  dj.tweens["music"] = flux.to(settings, time, {musicVolume = 0}):oncomplete(
    function()
      dj.stop("music")
...
function dj.fadeAmbience(time)
  if not time then time = 1 end
  if settings.ambienceMaxVolume <= 0 then return end
  dj.tweens["ambience"] = flux.to(settings, time, {ambienceVolume = 0}):oncomplete(
    function()
      dj.stop("ambience")
...
function dj:update(dt)
  --dj.volume("effect", settings.soundVolume * settings.soundMultiplier) -- this is done in dj.play now
  dj.volume("music", settings.musicVolume * settings.musicMultiplier)
  dj.volume("altMusic", settings.musicMaxVolume) -- alt is always max, no need for fade
  dj.volume("ambience", settings.ambienceVolume * settings.ambienceMultiplier)
end
